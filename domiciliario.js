document.addEventListener('DOMContentLoaded', () => {
    ...
    const supabase = window.supabase.createClient(config.supabaseUrl, config.supabaseKey, { auth: { storageKey: 'deliciasexpress-rider-auth' } });
    ...
    function subscribeToOrders() {
        ...
        ordersChannel = supabase
            ...
            .subscribe((status, err) => {
                console.log('[Realtime domiciliario]', status, err || '');
                if (status === 'CHANNEL_ERROR' || status === 'TIMED_OUT' || status === 'CLOSED') {
                    setTimeout(() => { if (currentUserId) subscribeToOrders(); }, 1500);
                }
            });

        // Respaldo obligatorio: si Realtime/RLS/WebSocket falla, el domiciliario
        // sigue viendo pedidos nuevos y cambios de sus entregas.
        ordersPollingInterval = setInterval(async () => {
            if (!currentUserId) return;
            const before = new Set(availableOrders.map(o => o.id));
            await loadAvailableOrders();
            await loadMyDeliveries();
            const hasNew = availableOrders.some(o => !before.has(o.id));
            if (hasNew) availableTab.classList.add('has-notification');
        }, 5000);
    }
    ...
    availableOrdersContainer.addEventListener('click', async (e) => {
        ...
        if (error || !(data && data.ok)) {
            ...
            } else if (data && data.error === 'ya_tiene_pedido_activo') {
                alert('Ya tienes un pedido en camino. Debes entregarlo antes de aceptar otro.');
            } else if (data && data.error) {
                alert('No se pudo aceptar el pedido: ' + data.error + (data.role_detectado ? ' (rol detectado: ' + data.role_detectado + ')' : ''));
            } else if (error) {
