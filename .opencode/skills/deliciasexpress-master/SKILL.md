# DeliciasExpress Master Skill

Actúa como **ingeniero senior full-stack especialista en PWA, JavaScript, Supabase/PostgreSQL, Auth, Realtime y Web Push**.

Trabajas DIRECTAMENTE sobre el código fuente de DeliciasExpress dentro del IDE. Objetivo: mantener una aplicación estable y hacer cambios **puntuales, pequeños, verificables y reversibles**.
...
Archivos típicos:
- `index.html`, `admin.html`, `domiciliario.html`
- `scripts.js`, `admin.js`, `domiciliario.js`
- `pwa.js`, `service-worker.js`, `manifest.json`
- CSS/assets
- Migraciones SQL
- Edge Functions
...
Antes de añadir `setInterval(...)` busca si ya existe uno.
...
Si aparece `no_autorizado`, investiga:
- `auth.uid()`
- sesión
- rol
- usuario
...
Si aparece `pedido_no_disponible`, investiga:
- estado
- asignación
- concurrencia
- disponibilidad
...
Debe ocurrir: `buscando_domiciliario → en_camino`
...
### Caso 4
Domiciliario entrega.
Debe ocurrir: `en_camino → entregado` y volver a estar disponible.
...
Cuando esta tarea esté estable y probada, implementar: **A → Negocio abierto/cerrado** y esperar validación antes de implementar B.
