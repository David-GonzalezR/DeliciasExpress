document.addEventListener('DOMContentLoaded', () => {
    ...
    const supabase = window.supabase.createClient(config.supabaseUrl, config.supabaseKey, { auth: { storageKey: 'deliciasexpress-client-auth' } });
    ...
    const TRACK_STATUS_MAP = {
        ...
        buscando_domiciliario: 'preparando' // buscar domiciliario no significa que el pedido ya salió
    };
    ...
    function loadFromLocalStorage() {
        ...
        if (savedCart) {
            try {
                ...
                cart = Array.isArray(parsedCart) ? parsedCart.filter(item => item && item.id && Number(item.quantity) > 0).map(item => ({
                    ...item,
                    quantity: Number(item.quantity),
                    selectedCustomizations: Array.isArray(item.selectedCustomizations) ? item.selectedCustomizations : [],
                    instructions: item.instructions || '',
                    finalPricePerUnit: Number(item.finalPricePerUnit || 0)
                })) : [];
            } catch (e) {
                console.warn('Carrito local corrupto; se reiniciará.', e);
                cart = [];
    ...
    function captureGpsLocation() {
        ...
        navigator.geolocation.getCurrentPosition(
            ...
            deliveryAddressInput.dataset.gpsLat = String(capturedLat);
            deliveryAddressInput.dataset.gpsLng = String(capturedLng);
            },
            (err) => {
    ...
    async function continueOrderFlow(orderId, address, total, orderItems) {
        ...
        if (window.DeliciasPush) window.DeliciasPush.linkOrder(orderId);
    ...
    async function refreshTrackedOrders() {
        for (const id of trackedOrderIds) {
            ...
            if (!error && data && data.length > 0) {
                ...
                orderCustomerRatings[id] = data[0].customer_rating ?? null;
                orderAcknowledged[id] = data[0].acknowledged_at ?? null;
            orderRiderIds[id] = data[0].assigned_rider_id ?? null;
            }
        }
        renderOrderStatusList();
        if (selectedOrderId) {
    ...
    async function refreshSingleOrderStatus(orderId) {
        ...
        if (!error && data && data.length > 0) {
            ...
            orderCustomerRatings[orderId] = data[0].customer_rating ?? null;
            orderAcknowledged[orderId] = data[0].acknowledged_at ?? null;
            orderRiderIds[orderId] = data[0].assigned_rider_id ?? null;
            renderOrderStatusList();
    ...
    function subscribeToOrderStatus(orderId) {
        ...
        orderRealtimeChannel = supabase
            ...
            .on('postgres_changes', {
                ...
                if (payload.new.customer_rating !== undefined) orderCustomerRatings[orderId] = payload.new.customer_rating;
                if (payload.new.acknowledged_at !== undefined) orderAcknowledged[orderId] = payload.new.acknowledged_at;
                if (payload.new.assigned_rider_id !== undefined) orderRiderIds[orderId] = payload.new.assigned_rider_id;
                renderOrderStatusList();
                if (selectedOrderId === orderId) {
            ...
            .subscribe((status, err) => {
                console.log('[Realtime cliente]', status, err || '');
            });
        ...
        orderPollingInterval = setInterval(() => refreshSingleOrderStatus(orderId), 3000);
    }
    ...
    function renderOrderStatus(status, orderId) {
        ...
        if (status === 'entregado') stopOrderPolling();
        updateConfirmReceiptButton();
        loadOrderRiderInfo(status, orderId);
        renderCustomerRating(status, orderId);
    }
    ...
    function updateConfirmReceiptButton() {
        ...
        const acknowledged = !!orderAcknowledged[selectedOrderId];
        const canConfirm = status === 'en_camino' || (status === 'entregado' && !acknowledged);
        if (selectedOrderId && canConfirm) {
            confirmReceiptBtn.style.display = 'inline-flex';
            confirmReceiptBtn.disabled = false;
            confirmReceiptBtn.textContent = status === 'entregado'
    ...
    function renderCustomerRating(status, orderId) {
        const existing = document.getElementById('customer-rating-panel');
        if (existing) existing.remove();
        if (!orderRiderInfo || status !== 'entregado' || !orderRiderIds[orderId] || !orderAcknowledged[orderId]) return;

        const rating = orderCustomerRatings[orderId];
        if (rating != null) return;

        const panel = document.createElement('div');
        panel.id = 'customer-rating-panel';
        panel.className = 'customer-rating-panel';
        panel.innerHTML = `
            <h4>¿Cómo fue tu experiencia?</h4>
            <p>Califica al domiciliario que entregó tu pedido.</p>
            <div class="customer-rating-stars" role="group" aria-label="Calificar domiciliario">
                ${[1,2,3,4,5].map(n => `<button type="button" class="rating-star" data-rating="${n}" aria-label="${n} estrella${n > 1 ? 's' : ''}">★</button>`).join('')}
            </div>
            <div id="customer-rating-message" class="customer-rating-message"></div>
        `;
        orderRiderInfo.insertAdjacentElement('afterend', panel);
        panel.addEventListener('click', submitCustomerRating);
    }

    async function submitCustomerRating(e) {
        const btn = e.target.closest('.rating-star');
        if (!btn || !selectedOrderId) return;
        const rating = Number(btn.dataset.rating);
        const panel = document.getElementById('customer-rating-panel');
        if (!panel) return;
        panel.querySelectorAll('.rating-star').forEach(b => b.disabled = true);
        const message = panel.querySelector('#customer-rating-message');
        const { data, error } = await supabase.rpc('submit_order_rating', {
            p_order_id: selectedOrderId,
            p_rating: rating
        });
        if (error || !(data && data.ok)) {
            console.error('Error calificando domiciliario:', error || data);
            panel.querySelectorAll('.rating-star').forEach(b => b.disabled = false);
            const reason = data?.error ? ` (${data.error})` : '';
            if (message) message.textContent = `No pudimos guardar la calificación. Intenta de nuevo.${reason}`;
            return;
        }
        orderCustomerRatings[selectedOrderId] = rating;
        if (message) message.textContent = '¡Gracias por calificar!';
        panel.querySelectorAll('.rating-star').forEach(b => {
            b.classList.toggle('selected', Number(b.dataset.rating) <= rating);
        });
        const ratedOrderId = selectedOrderId;
        setTimeout(() => { if (selectedOrderId === ratedOrderId) renderCustomerRating('entregado', ratedOrderId); }, 1200);
    }
    ...
    async function confirmOrderReceived() {
        ...
        orderStatuses[selectedOrderId] = 'entregado';
        orderAcknowledged[selectedOrderId] = new Date().toISOString();
        confirmReceiptBtn.style.display = 'none';
        showCustomAlert('¡Gracias! Tu pedido fue marcado como entregado.');
        await refreshTrackedOrders();
        renderCustomerRating('entregado', selectedOrderId);
    }
