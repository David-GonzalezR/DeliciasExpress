...
> **Estado: ✅ RESUELTA** — Todos los hallazgos fueron aplicados en la base de datos el 2026-08-15.
...
* **Estado**: ✅ Resuelto — RLS habilitado con políticas: "Clientes ven sus propios items", "Admins ven todos los items" y "Cualquiera puede crear items de pedido" (necesario para pedidos anónimos).
* **Gravedad**: Crítica
...
* **Estado**: ✅ Resuelto — `handle_new_user()` guarda `NEW.email` en `public.profiles`.
* **Gravedad**: Alta
...
* **Estado**: ✅ Resuelto — Trigger `trigger_check_role_change` con función `prevent_role_escalation()` que bloquea cambios de rol a no-admins (permite al panel admin y al usuario `postgres` del dashboard).
* **Gravedad**: Media
...
* **Estado**: ✅ Resuelto — La función `get_order_status(uuid)` existe como `SECURITY DEFINER` y devuelve `id, status, total, created_at, updated_at`, permitiendo a clientes anónimos consultar su pedido por UUID.
* **Gravedad**: Baja
