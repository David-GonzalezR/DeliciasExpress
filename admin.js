document.addEventListener('DOMContentLoaded', () => {
    ...
    const supabase = window.supabase.createClient(config.supabaseUrl, config.supabaseKey, { auth: { storageKey: 'deliciasexpress-admin-auth' } });
    ...
    ordersContainer.addEventListener('click', async (e) => {
        ...
        if (btn.dataset.action === 'advance') {
            ...
            const { data: requestResult, error } = await supabase.rpc('request_delivery', { p_order_id: orderId });
            if (!error && requestResult && requestResult.ok === false) {
                alert(requestResult.error || 'No se pudo solicitar el domiciliario.');
                btn.disabled = false;
                return;
            }
            if (error) {
